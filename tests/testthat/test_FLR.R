...
res <- EstFLM(y, X, model = list(type = "smoothspline", rho = 0.00000001), intercept = FALSE)
...
test_that("EstFLM", {
    expect_error(EstFLM())
    expect_error(EstFLM(2,5,model = list(type = "abc",rho = 3)))
})
...
test_that("Estimation", {
    ...
    ## estimate, given rho
    res1 <- EstFLM(y, X, model = list(type = "smoothspline", rho = 0.00000001), intercept = TRUE)
    expect_equal(mean(res1$residuals), 0)

    ## estimate, given df
    res2 <- EstFLM(y, X, model = list(type = "smoothspline", df = 6), intercept = TRUE)
...
test_that("Global Test: plausible values",{
    ...
    expect_true(GlobalTest(res, alternative = beta)$pval>0.0001)
})
